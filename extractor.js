...
function getBrowserExecutable() {
  ...
  throw new Error(
    'No compatible browser (Google Chrome or Microsoft Edge) found on system.'
  );
}
 ...
 * }>}
 */
export async function extractFacebookPhotos(
  rawUrl,
  options = {},
  onProgress = () => {}
) {
...
const executablePath = isVercel
  ...
  function addImage(
    ...
    if (
      ...
      cleanUrl.includes('/static.xx.')
    ) {
      return;
    }

    if (
      cleanUrl.includes('/t39.30808-1/') ||
      cleanUrl.includes('/p50x50/') ||
    ...
    const isHigherQuality =
      !existing ||
      (cleanUrl.includes('t39.30808-6') &&
        !existing.url.includes('t39.30808-6')) ||
    ...
    targetPage.on('response', async (response) => {
      ...
      if (resUrl.includes('/api/graphql/')) {
        try {
          ...
          const largestRegex =
            /"id":"([0-9]+)".*?"largest_available_image":\{"uri":"([^"]+)"(?:,"width":(\d+),"height":(\d+))?/g;
          ...
          const imageRegex =
            /"id":"([0-9]+)".*?"image":\{"uri":"([^"]+)"(?:,"width":(\d+),"height":(\d+))?/g;
  ...
  try {
    ...
    onProgress({
      step: 'navigating',
      message: 'Connecting to Facebook post...',
      percent: 20
    });
    ...
    onProgress({
      step: 'parsing_scripts',
      message: 'Scanning preloaded media...',
      percent: 35
    });
    ...
    const scriptImages =
      (await safeEvaluate(page, () => {
        ...
        for (const s of scripts) {
          ...
          const uriRegex =
            /"uri":"(https:[^"]+scontent[^"]+)"(?:,"width":(\d+),"height":(\d+))?/g;
    ...
    // Discover clean photo viewer link from post
    onProgress({
      step: 'discovering_viewer',
      message: 'Locating album and photo links...',
      percent: 45
    });
    ...
    let cleanViewerLink = null;

for (let attempt = 0; attempt < 5; attempt++) {
  cleanViewerLink = await safeEvaluate(page, () => {
    const anchors = Array.from(
      document.querySelectorAll(
        'a[href*="/photo"], a[href*="photo.php"]'
      )
  ...
  if (cleanViewerLink) {
    console.log(`Photo viewer found: ${cleanViewerLink}`);
    break;
  }

  await new Promise((r) => setTimeout(r, 1000));
}
    const isAlreadyInViewer =
      targetUrl.includes('/photo/?fbid=') ||
      targetUrl.includes('/photo.php?fbid=');

    const startViewerUrl =
      cleanViewerLink ||
      (isAlreadyInViewer ? targetUrl : null);

    if (startViewerUrl) {
      onProgress({
        step: 'opening_viewer',
        message: 'Opening dedicated photo viewer in fresh tab...',
        percent: 55
      });
      ...
      if (!isAlreadyInViewer) {
        ...
        await viewerPage.goto(startViewerUrl, {
          waitUntil: 'domcontentloaded',
          timeout: 35000
        });
      ...
      try {
        await safeEvaluate(viewerPage, () => {
          document
            .querySelectorAll(
              '[aria-label="Close"], [aria-label="Decline optional cookies"], [aria-label="Allow all cookies"]'
            )
            .forEach((b) => {
              try {
                b.click();
      ...
      // Walk through all photos in the album/post
      onProgress({
        step: 'walking_carousel',
        message: 'Walking through all album photos...',
        percent: 60
      });
      ...
      for (let step = 0; step < maxWalkSteps; step++) {
        ...
        if (currentFbid) {
          if (firstFbid === null) {
            ...
            currentFbid === firstFbid &&
            step > 2
          ) {
            console.log(
              `Loop detected at step ${step} (returned to ${firstFbid}). Album walk complete!`
            );
        ...
        if (currentUrl === lastUrl && step > 0) {
          ...
          if (unchangedUrlCount >= 3) {
            console.log(
              `URL stopped changing at step ${step}. Reached end of photos.`
            );
        ...
        const activeImg = await safeEvaluate(viewerPage, () => {
          ...
          if (vc) {
            return {
              src: vc.src,
              width: vc.naturalWidth,
              height: vc.naturalHeight
            };
          }
        ...
        onProgress({
          ...
          percent: Math.min(
            94,
            60 + Math.floor((step / maxWalkSteps) * 34)
          ),
        ...
        const clickedNext = await safeEvaluate(viewerPage, () => {
          const btn = document.querySelector(
            '[aria-label="Next photo"], [aria-label="Next"], [aria-label="next"]'
          );
    ...
    onProgress({
      step: 'finalizing',
      message: 'Organizing photo gallery...',
      percent: 96
    });
